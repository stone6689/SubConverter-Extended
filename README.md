...
从单纯服务于 Mihomo 内核的客户端的角度来讲，完全可以以令订阅转换后端在配置文件中生成符合内核配置文件格式要求的 `proxy-provider` 字段，取代过去的读取订阅>写入节点参数的功能，从而完全规避工具自身对节点参数的支持滞后以及订阅服务商的限制问题。  

同样，对于节点链接的解析，也完全可以直接引入 Mihomo 内核的解析器模块，来替代过去的人工维护解析器，使订阅转换后端的节点链接解析能力与 Mihomo 内核完全对齐。  
